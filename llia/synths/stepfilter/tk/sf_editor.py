...
FRQLST = [100,150,200,300,400,600,800,1200,
          1600,2400,3200,4800,6400,9600,12800]
...
class TkStepFilterPanel1(TkSubEditor):
    ...
    def __init__(self, editor):
        ...
        lab_panel = factory.image_label(self, "resources/StepFilter/editor.png")
        lab_panel.pack(anchor="nw", expand=False)
        # Clock
        s_clock = cfactory.simple_lfo_freq_slider(self, "clockFreq", editor)
        r_sliders = []
        r_labels = []
        a_sliders = []
        b_sliders = []
        ...
        for i in range(8):
            ...
            ak = "a%d" % j
            bk = "b%d" % j
            rslide = cfactory.discrete_slider(self, rk, editor, r_values, ttip=rk)
            rslide.index = i
            aslide = cfactory.discrete_slider(self, ak, editor, mix_values, ttip=ak)
            bslide = cfactory.discrete_slider(self, bk, editor, mix_values, ttip=bk)
        ...
        s_alag = cfactory.normalized_slider(self, "aLag", editor, "A Lag time")
        s_amin = cfactory.discrete_slider(self, "aMin", editor, FRQLST, "Minimum A frequency")
        s_amax = cfactory.discrete_slider(self, "aMax", editor, FRQLST, "Maximum A frequency")
        s_ares = cfactory.normalized_slider(self, "aRes", editor, "A Resonace")
        s_apan = cfactory.bipolar_slider(self, "aPan", editor, "A Pan position")
        s_apanlfo = cfactory.normalized_slider(self, "panLfoA", editor, "LFO -> A Pan")
        s_aamp = cfactory.volume_slider(self, "aAmp", editor, "A Filter amp")
        s_blag = cfactory.normalized_slider(self, "bLag", editor, "B Lag time")
        s_bmin = cfactory.discrete_slider(self, "bMin", editor, FRQLST, "Minimum B frequency")
        s_bmax = cfactory.discrete_slider(self, "bMax", editor, FRQLST, "Maximum B frequency")
        s_bres = cfactory.normalized_slider(self, "bRes", editor, "B Resonace")
        s_bpan = cfactory.bipolar_slider(self, "bPan", editor, "B Pan position")
        s_bpanlfo = cfactory.normalized_slider(self, "panLfoB", editor, "LFO -> B Pan")
        s_bpanratio = cfactory.linear_slider(self, "panLfoBRatio", editor, range_=(1, 8))
        s_bamp = cfactory.volume_slider(self, "bAmp", editor, "B Filter Amp")
        s_drypan = cfactory.bipolar_slider(self, "dryPan", editor, "Dry signal Pan")
        s_drypanlfo = cfactory.normalized_slider(self, "panLfoDry", editor, "LFO -> Dry pan")
        s_dryamp = cfactory.volume_slider(self, "dryAmp", editor, "Dry signal amp")
        s_amp = cfactory.volume_slider(self, "amp", editor, "Overall amp")

        s_lfo = cfactory.simple_lfo_freq_slider(self, "panLfoFreq", editor, "Panning LFO Frequency")
        s_blfo = cfactory.discrete_slider(self, "panLfoBRatio", editor, [0.5, 0.75, 1, 1.5, 2, 3, 4], "Panning LFO B FIlter ratio")
